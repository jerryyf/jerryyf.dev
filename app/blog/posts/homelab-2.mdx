...
A properly designed home network configuration is, I would argue, necessary if we're going to run servers (especially public facing ones), IoT devices, client devices in our home. Having all your devices on a "flat" network where any client connected can all communicate with each other really isn't optimal.
